...
<script type="text/javascript">
	...
	function queryparamToggle(queryparamid)
	{
		//$('#'+queryparamid).toggle();
		var queryparam = document.getElementById(queryparamid);
		var currentclass = queryparam.className;

		if (currentclass == 'queryparamhide') {
			removeClassName(queryparam, 'queryparamhide');
			addClassName(queryparam, 'queryparamshow');
		} else {
			removeClassName(queryparam, 'queryparamshow');
			addClassName(queryparam, 'queryparamhide');
		}
...
if($output['queryTotals']['count'] ==  0) {
	...
	echo '<table class="side" cellspacing="0">
		  ...
		  <tr><td><var>'.count($output['duplicateQueries']).'</var> <h4>Duplicates</h4></td></tr>
		 </table>
		<table class="main" cellspacing="0">';

		$class = '';
		foreach($output['queries'] as $queryindex => $query) {
			echo '<tr onclick="queryparamToggle(\'queryparam_'.$queryindex.'\')" title="Click to show Parameters">
				<td class="'.$class.'">
					<b>'.$query['time'].'</b> '.$query['sql'];

			if ($query['count'] > 1)
				echo '<i> ('.$query['count'].' times)</i></span>';


			echo '<div style="padding:10px 10px 10px 55px" class="queryparamhide" id="queryparam_'.$queryindex.'">';
			...
			if (count($query['params']) > 0) {
				...
				echo '<em>No parameter for this query</em>';
			}
