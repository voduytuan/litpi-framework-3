...
class PhpQuickProfiler {
	...
	public function gatherMemoryData() {
		...
		//Because of Hiphop, i changed it to memory_get_usage(true) instead of memory_get_peak_usage()
		//$memoryTotals['used'] = $this->getReadableFileSize(memory_get_peak_usage());
		$memoryTotals['used'] = $this->getReadableFileSize(memory_get_usage(true));
	...
	public function gatherQueryData() {
		...
		if($this->querydebug != '') {
			...
			foreach($this->querydebug->queries as $key => $query) {
				...
				for ($i = 0; $i < count($queries); $i++) {
					if ($queries[$i]['sql'] == $query['sql'] && $queries[$i]['params'] === $query['params']) {
						$appearCount = ++$queries[$i]['count'];
						if ($appearCount == 2) {
		...
		$this->output['duplicateQueries'] = $duplicateQueries;
	}
